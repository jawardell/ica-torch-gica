...
SUBJECTS_FILE=${PROJECT_DIR}/DEVCOG/subjects.txt
...
do
	cd ${PROJECT_DIR}/DEVCOG/$subject
	IFS=$'\n' sessions=(`ls -1`)
	for session in "${sessions[@]}"
	do
		if [ -d ${PROJECT_DIR}/DEVCOG/$subject/$session/processed ];then
			cd ${PROJECT_DIR}/DEVCOG/$subject/$session/processed
