...
def gigicar(FmriMatr, ICRefMax):
    ...
    eigenvalues, index = D[:, 0].sort(descending=True)
    Esort = E[:, index]
    dsort = eigenvalues
    ...
    WhitenMatrix = Epart.t() @ torch.linalg.inv(sqrtm(Lambda_part))
    ...
    for ICnum in range(EsICnum):
        ...
        for i in range(iternum):
            ...
            EYgy = (1 / m) * Y @ (torch.tanh(y1).reshape(1, dim)).t()
            Jy1 = (EGy1 - EGv)**2
            ...
            d = g / g_norm
            wx = wc.view(wc.shape[0], 1) + Nemda * d
            wx = wx / norm(wx)
            y3 = wx.t() @ Y
